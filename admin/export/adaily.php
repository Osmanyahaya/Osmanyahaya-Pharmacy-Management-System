<?php
	...
	";
				...
				$sumtotal = 0;
					$i = 1;
                    $query = $conn->query("SELECT tp.*,tl.code, tl.client_id,pl.prod_name as product,tl.date_created,sum(tp.qty) as qty FROM `transaction_products` tp inner join transaction_list tl on tp.transaction_id = tl.id inner join product pl on tp.product_id = pl.prod_id where date(tl.date_created) = '{$date}' group by pl.prod_id order by unix_timestamp(tl.date_updated) asc ") or die(mysqli_errno());
