        ...
        <div class="container-fluid" id="printout">
			<table class="table table-hover table-striped table-bordered">
				...
				<tbody>
                    ...
                    $totalqty=0;
					$i = 1;
                    $qry = $conn->query("SELECT tp.*,tl.code, tl.client_id,pl.prod_name as product,tl.date_created FROM `transaction_products` tp inner join transaction_list tl on tp.transaction_id = tl.id inner join product pl on tp.product_id = pl.prod_id where date(tl.date_created) BETWEEN '{$from}' and '{$to}' and prod_id= '{$prod}' order by tl.date_updated desc  ");
...
<script>
	$(document).ready(function(){
		$('#filter-form').submit(function(e){
            e.preventDefault()
            location.href = "./?page=reports/single_prod_sales_report&"+$(this).serialize()
        })
        $('#product_sel').select2({
