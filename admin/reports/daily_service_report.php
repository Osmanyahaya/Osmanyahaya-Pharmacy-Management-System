        ...
        <div class="container-fluid" id="printout">
			<table class="table table-hover table-striped table-bordered">
				...
				<tbody>
                    ...
                    $subtotal = 0;
					$i = 1;
                    $qry = $conn->query("SELECT ts.*,tl.code, tl.client_id,sl.name as `service`,sl.price as `price`,tl.date_created,count(ts.id) as qty  FROM `transaction_services` ts inner join transaction_list tl on ts.transaction_id = tl.id inner join service_list sl on ts.service_id = sl.id where date(tl.date_created) = '{$date}' group by sl.id order by unix_timestamp(tl.date_updated) asc ");
                ...
                <tfoot>
                    <th class="py-1 text-center" colspan="7">Total Service Price</th>
