        ...
        <div class="container-fluid" id="printout">
			<table class="table table-hover table-striped table-bordered">
				...
				<tbody>
                    ...
                    $total = 0;
					$i = 1;
                    $qry = $conn->query("SELECT *, sum(amount) as total from transaction_list where date(date_created) BETWEEN '{$from}' and '{$to}' and user_id='{$user}' group by date(date_created) order by unix_timestamp(date_updated)  ");
...
<script>
	$(document).ready(function(){
		$('#filter-form').submit(function(e){
            e.preventDefault()
            location.href = "./?page=reports/user_daily&"+$(this).serialize()
        })
        $('#print').click(function(){
            var h = $('head').clone()
            var ph = $($('noscript#print-header').html()).clone()
            var p = $('#printout').clone()
            h.find('title').text('User Daily Sales Report - Print View')
